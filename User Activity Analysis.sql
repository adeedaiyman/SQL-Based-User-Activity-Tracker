...
SELECT USERS.USER_ID, USERS.USER_NAME, MAX(LOGIN_TIMESTAMP) AS LAST_ACTIVE
FROM USERS
JOIN LOGINS ON USERS.USER_ID = LOGINS.USER_ID
WHERE MONTH(CURRENT_TIMESTAMP-LOGIN_TIMESTAMP) > 5 AND USER_STATUS = 'Active'
...
WITH CTE AS (
SELECT USER_ID, SESSION_ID, YEAR(LOGIN_TIMESTAMP) AS YEAR ,MONTH(LOGIN_TIMESTAMP) AS MONTH,
CASE WHEN MONTH(LOGIN_TIMESTAMP) >= 1 AND MONTH(LOGIN_TIMESTAMP) <= 3 THEN 'Q1'
WHEN MONTH(LOGIN_TIMESTAMP) >= 4 AND MONTH(LOGIN_TIMESTAMP) <= 6 THEN 'Q2'
WHEN MONTH(LOGIN_TIMESTAMP) >= 7 AND MONTH(LOGIN_TIMESTAMP) <= 9 THEN 'Q3'
WHEN MONTH(LOGIN_TIMESTAMP) >= 10 AND MONTH(LOGIN_TIMESTAMP) <= 12 THEN 'Q4'
END AS QUARTER
FROM LOGINS)
SELECT YEAR, QUARTER, COUNT(USER_ID) AS USERS, COUNT(SESSION_ID) AS SESSION
FROM CTE 
GROUP BY YEAR, QUARTER


select datepart(quarter, LOGIN_TIMESTAMP) AS QUARTER, COUNT(DISTINCT(USER_ID)) AS TOTAL_USERS, COUNT(SESSION_ID) AS TOTAL_SESSIONS
FROM LOGINS
...
FROM LOGINS
WHERE LOGIN_TIMESTAMP BETWEEN '2024-01-01' AND '2024-01-30'  AND
...
WITH CTE AS(
select datetrunc(quarter, min(LOGIN_TIMESTAMP)) AS FIRST_QUARTER_DATE, COUNT(DISTINCT(USER_ID)) AS TOTAL_USERS, COUNT(SESSION_ID) AS TOTAL_SESSIONS
FROM LOGINS
GROUP BY datetrunc(quarter, LOGIN_TIMESTAMP) 
), CTE1 AS (
SELECT *,
LAG(TOTAL_SESSIONS) OVER(ORDER BY  FIRST_QUARTER_DATE) AS PREV_SES_CNT
FROM CTE
)
SELECT *,
(((TOTAL_SESSIONS - PREV_SES_CNT)*100)/PREV_SES_CNT) as prcnt_chng
FROM CTE1
...
SELECT *,
ROW_NUMBER() OVER(PARTITION BY DATE ORDER BY USER_ID) AS RN
FROM CTE)
...
DATEDIFF(DAY, MIN(CAST(LOGIN_TIMESTAMP AS DATE)), GETDATE()) AS NO_OF_DAYS_LOGIN_REQUIRED,
COUNT(DISTINCT CAST(LOGIN_TIMESTAMP AS DATE)) AS TOTAL_LOGIN_DAYS
FROM LOGINS
GROUP BY USER_ID
HAVING DATEDIFF(DAY, MIN(CAST(LOGIN_TIMESTAMP AS DATE)), GETDATE()) = COUNT(DISTINCT CAST(LOGIN_TIMESTAMP AS DATE))
ORDER BY USER_ID;
